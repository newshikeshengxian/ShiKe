 ...
 * 添加商品到购物车 
 */
function addToCartTopic(goodsId){
	...
	Ajax.call('flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponseTopic, 'POST', 'JSON');
}

/* *
 * 处理添加商品到购物车的反馈信息
 */
function addToCartResponseTopic(result){
	if (result.error > 0){
	    // 如果需要缺货登记，跳转
	    if (result.error == 2){
		...
		var cart_url = 'templates.index.php';
		if (cartInfo){
		...
		if (result.one_step_buy == '1'){
			...
			switch(result.confirm_type){
		        case '1' :
		          //if (confirm("该商品已添加到购物车，您现在还需要继续购物吗？\n如果您希望返回首页，请点击“确定”按钮。\n如果您希望继续购物，请点击“取消”按钮。")) location.href = cart_url;
		        	show_msg_div();
		          break;
		        case '2' :
		          //if (!confirm("该商品已添加到购物车，您现在还需要继续购物吗？\n如果您希望返回首页，请点击“确定”按钮。\n如果您希望继续购物，请点击“取消”按钮。")) location.href = cart_url;
...
var bool=true;
var divs=document.getElementById("add_to_cart_success");
function show_msg_div(){
	if(bool){
		bool=false;
		$("#add_to_cart_success").addClass("fade-div");
		setTimeout(function(){
			$("#add_to_cart_success").removeClass("fade-div");
			bool=true;
		},3000)
	}
}
...
function addToCart() {
	var goodsId = addToCart.arguments[0];
	var parentId = addToCart.arguments[1];
	var isnowbuy = addToCart.arguments[2];
	var extCode = addToCart.arguments[3];
	var extId = addToCart.arguments[4];
	var goods = new Object();
	var spec_arr = new Array();
	var fittings_arr = new Array();
	var number = 1;
	...
	if (isSelectAttr(spec_arr)) {
        ...
        Ajax.call('flow.php?step=add_to_cart', 'goods=' + JSON.stringify(goods) +'&actID='+extId, addToCartResponse, 'POST', 'JSON');
	}
}
...
function addToCartResponse(result) {
	if (result.error > 0) {
		...
		if (result.error == 1){
            ...
            $('.pop-compare .pop-text').html("对不起，该商品已经库存不足暂停销售!").css('padding-top',10);
		...
		if (result.one_step_buy == '1') {
			...
			if(result.ms_goods == 9 ){
				$('.pop-mask,.pop-compare').show();
				$('.pop-compare .pop-text').html('秒杀活动只能参加一次！').css('padding-top',10);
				$('.pop-compare').css({'top':380});
			}else{
...
* popup 公用函数，弹窗
* title content 内容，文字
* funcSuc, fucClose 传入方法
*/
function popup(title,content,funcSuc,funcClose){
	//初始化
	var title = title || "提示框";
	var content = content || "确认么？";
	var funcSuc = funcSuc || function(){};
	var funcClose = funcClose || function(){};
	
	//拼接弹窗div
	var vhtml ='<div class="pop-main" id="popfuc">';
		vhtml+='<div class="pop-header"> <span>'+ title +'</span><a class="pop-close" href="javascript:;" title="关闭"></a></div>';
		vhtml+='<div class="pop-con">';
		vhtml+= content;		   
		vhtml+= '</div></div>';
		
	$("body").append(vhtml);	
	$('#popfuc,.pop-mask').show();
	$('#popfuc').css('top',($(window).height()-$('#popfuc').height())/2);		
	$('#popfuc .pop-close').click(function(){
		$('#popfuc,.pop-mask').hide();
	});
	$('#popfuc .pop-close-shike').click(function(){
		$('#popfuc,.pop-mask').hide();
	});
...
function Tabs(ulID){
	this.$ul = $(ulID) || $("ul.tabs") 
	var $ul = this.$ul;
	this.$li = $ul.find('li');
	var $li = this.$li;
	this.eachWidth = parseInt($li.width()) + parseInt($li.css('margin-left')) + parseInt($li.css('margin-right')) + parseInt($li.css('padding-left')) + parseInt($li.css('padding-right')) + parseInt($li.css('border-left')) + parseInt($li.css('border-right'));
	//current 范围 在0～ total 之间
	this.current = 0;
	this.total = $ul.find('li').length;
	this.move = "left";
	this.middle = 3;
	//初始化
	this.init();
...
Tabs.prototype.moveLeft = function(){
	
	if(this.current < 1){
		return false;
	}
	else{
		var nth = this.current - 1;
		this.current = nth;
		var that = this;
		console.log('left',nth);
		this.$ul.animate({'left': (-1)*nth*that.eachWidth},500);
	}
	
};
Tabs.prototype.moveRight = function(){
	
	if(this.current < this.total - this.middle){
		var nth = this.current + 1;
		this.current = nth;
		var that = this;
		console.log('right',nth);
...
//删除选中商品
function deleteCartGoodsArray(rec_id_arr){
	var rec_id_arr = [].concat(rec_id_arr);
	console.log(rec_id_arr);
	for(var i=0; i < rec_id_arr.length; i++){
		
		Ajax.call('delete_cart_goods.php', 'id=' + rec_id_arr[i], deleteCartGoodsResponse02, 'POST', 'JSON');
	}
}
 ...
 * 接收返回的信息
 */
function deleteCartGoodsResponse02(res){
