$(document).ready(function() {
    ...
    function change_widget(i, obj){
		...
		if(i==2){
			$("#main_widget_2").show();
			var top = parseInt($("#wrapper").offset().top)- 100;
			$("html,body").animate({
				scrollTop: top
			},
			500);
	...
	setTimeout(function(){
		if($('#nav .boldtit-list').length){
			$('#main_widget_1>.mc').css('display','none');
			
			$('li.boldtit-list').click(function(){
				var nth = $('li.boldtit-list').index($(this));	
				if(nth == 1){
					$('#os_jieshao').fadeIn('slow');
					$('.detail-content').css("display","none");
				}
				else if(nth == 0){
					$('#os_jieshao').css("display","none");	
					$('.detail-content').fadeIn('slow');
				}
				
			});
			
			//$('#main_widget_1>.mc').eq(0).css('display','none');
		}
	},500);
