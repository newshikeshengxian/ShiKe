...
function banner_play(a,b,c,d){
	...
	if(blength > 1){
		$(b).mouseover(function(){
			...
			$(a).eq($(this).fsaa()).hide().fadeIn().siblings().fadeOut();
...
$('.category-wrap .slide-aside').click(function(){
	if($('.category-wrap .aside').width() == 0){
		...
		$(".slide-aside").background="url(../images/slide-right.png) no-repeat 0 0";
		document.querySelector(".slide-aside").style.background="url(themes/68ecshopcom_360buy/images/slide-aside.gif) 2px 2px";
	}else{
		...
		document.querySelector(".slide-aside").style.background="url(themes/68ecshopcom_360buy/images/slide-right.png)  2px 2px";
	}
	if($('.main').hasClass('main1210')){
		$('#compareBox').css({'left':Math.ceil(($(window).width()-990)/2)})	
	}else{
...
function tab_change(tab,cur,con){
	$(tab).click(function(){
		...
		$(con).eq($(this).fsaa()).show().siblings(con).hide();
	})
};
